webshop.ProductView = class {
	...
	*/
	constructor(options) {
		...
		this.order_by_select_field = $("#order-by-select-field");
		this.ordering = this.order_by_select_field.val();
		this.make();
	}
	...
	prepare_toolbar() {
		...
		// 	</div>
		// `);
		// this.searchInputEl = $(".search-bar .search-bar-input");
	...
	render_grid_view(items, settings) {
		...
		new webshop.ProductGrid({
			items: items,
			products_section: me.products_section,
			settings: settings,
	...
	get_query_filters() {
		...
		if(this.ordering) {
			additional_data["order_by"] = this.ordering;
		}

		if(search) {
			additional_data["search"] = search;
		}
	...
	add_paging_section(settings) {
		...
		if (this.products) {
			...
			const pages_count = Math.ceil(this.product_count / page_length);

			paging_html += `
				<button class="btn btn-primary btn-prev" data-start="${start - page_length}"
					style="float: left" ${prev_disable}>
					${__("Prev")}
				</button>`;
			console.log("pages_count", pages_count, this.product_count, page_length);
			for (let i = 0; i < pages_count; i++) {
				let page_start = i * page_length;
				let active_class = start === page_start ? "active" : "";
				paging_html += `
					<li class="page-item ${active_class}">
						<a class="page-link" data-start="${page_start}">${i + 1}</a>
					</li>
				`;
			}

			paging_html += `
				<button class="btn btn-primary btn-next" data-start="${start + page_length}"
					${next_disable}>
					${__("Next")}
				</button>
			`;
			...
			$(".product-paging-area").html(paging_html);
	...
	bind_paging_action() {
		...
		const pagingAreaEl = $(".product-paging-area");
		pagingAreaEl.find('.btn-prev, .btn-next').click((e) => {
			const $btn = $(e.target);
		...
		pagingAreaEl.find('.page-link').click((e) => {
			const $btn = $(e.target);
			me.from_filters = false;
	...
	bind_filters() {
		...
		this.order_by_select_field.on('change', (e) => {
			me.ordering = e.target.value;
			me.change_route_with_filters();
		});
