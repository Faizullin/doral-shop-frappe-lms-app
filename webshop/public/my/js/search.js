var GloabalProductSearch = class {
	init(opts) {
		/* Options: search_box_id (for custom search box) */
		$.extend(this, opts);
		this.MAX_RECENT_SEARCHES = 4;
		this.searchBox =  $(".global-search-box");
        this.searhcBoxInput = this.searchBox.find(".search-input");
	...
	bindSearchAction() {
		let me = this;
        this.searchBox.on("submit", (e) => {
            e.preventDefault();
            let query = me.searhcBoxInput.val();
...
frappe.provide("webshop.webshop.gloabl_search");
var gloabl_search = webshop.webshop.gloabl_search;
gloabl_search.instance = new GloabalProductSearch();

$(document).ready(function () {
    gloabl_search.instance.init();
});