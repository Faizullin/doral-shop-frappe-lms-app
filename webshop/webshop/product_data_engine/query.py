...
class ProductQuery:
	...
	def query(self, attributes=None, fields=None, search_term=None, start=0, item_group=None, order_by=None):
		...
		self.set_ordering(order_by = order_by)
	...
	def set_ordering(self, order_by = None):
		order_options = {
			'newest': 'creation desc',
			'popular': 'ranking desc',
			'price_low_high': 'raw_price_value asc',
			'price_high_low': 'raw_price_value desc',
			'name_az': 'item_name asc',
			'name_za': 'item_name desc'
		}
		self.order_by = order_options.get(order_by, 'ranking desc')
